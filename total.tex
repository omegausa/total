...
\begin{theorem}
  ...
  Let $n$ be a positive integer and let $\rho$ be the partition $(n-1,n-2,\dots,1)$.
  For a partition $\mu$ of length $n$, let
  \[ a_\mu = \det(x_i^{\mu_j})_{1 \le i,j \le n} \]
  and define the Schur function
  \[ s_\mu = \sum_\tau x_1^{\gamma(\tau)_1}\dots x_n^{\gamma(\tau)_n}, \]
  where the sum is taken over semistandard Young tableaux $\tau$ on $\mu$
  with entries in $\{1,\dots,n\}$ and $\gamma(\tau)$ is the content of $\tau$.
  Then for all partitions $\lambda$ of length $n$,
